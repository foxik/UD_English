...
my @sentence_starts = (0) x length $input;
my @sentence_ends = (0) x length $input;
for (my $i = 0; $i < length $input && substr($input, $i, 1) =~ /\s/; $i++) { $sentence_ends[$i] = 1; }
for (my $i = length($input) - 1; $i >= 0 && substr($input, $i, 1) =~ /\s/; $i--) { $sentence_starts[$i] = 1; }
...
for (my $i = 0; $i < length $input; $i++) {
  ...
  for (my $j = $i; $j < length $input; $j++) {
    ...
    if (exists $sentences{$sentence}) {
      $sentence_starts[$i] = 1; for (my $k = $i - 1; $k >= 0 && substr($input, $k, 1) =~ /\s/; $k--) { $sentence_starts[$k] = 1; }
      $sentence_ends[$j] = 1; for (my $k = $j + 1; $k < length $input && substr($input, $k, 1) =~ /\s/; $k++) { $sentence_ends[$k] = 1; }
    }
  }
}
...
for (my $i = 0; $i < length $input; $i++) {
  ...
  for (my $j = $i; $j < length $input; $j++) {
    ...
    $sentence .= substr $input, $j, 1;
    last if not exists $sentence_prefixes{$sentence};
    if (exists $sentences{$sentence} && ($j + 1 == length $input || $sentence_starts[$j+1])) {
      ...
      foreach my $range (@{$sentences{$sentence}->{ranges}}) {
        ...
        my $raw = substr($input, $i, $j - $i + 1 + 1); # Add one additional character for SpaceAfter of last token
